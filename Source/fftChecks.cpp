...
fftChecks::plotCoRoutine()
{
	while (true)
	{
		if (_2i2_toggleButton->getToggleState())
		{
			doSine();
			co_yield 1;
...
void fftChecks::doFMTime()
{
	...
	m_plotTimeFM->plot({ y_data }, { x_ticksTime });
...
void fftChecks::resizePlotWindow()
{
	...
	auto bnds = getBounds();
	auto defaultsTextButtonY = defaultsTextButton->getY();

	auto heights = defaultsTextButtonY / noPlotsActive;
