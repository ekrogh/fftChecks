    ...
    : pFFTChecks(pFFTChks)
{
    ...
    signalToggleButton.reset (new juce::ToggleButton ("signalToggleButton"));
    addAndMakeVisible (signalToggleButton.get());
    signalToggleButton->setButtonText (TRANS("signal"));
    signalToggleButton->addListener (this);

    signalToggleButton->setBounds (0, 257, 95, 24);

    carrierToggleButton.reset (new juce::ToggleButton ("carrierToggleButton"));
    addAndMakeVisible (carrierToggleButton.get());
    carrierToggleButton->setButtonText (TRANS("carrier"));
    carrierToggleButton->addListener (this);

    carrierToggleButton->setBounds (0, 285, 95, 24);

    FFTToggleButton.reset (new juce::ToggleButton ("FFTToggleButton"));
    addAndMakeVisible (FFTToggleButton.get());
    FFTToggleButton->setButtonText (TRANS("FFT"));
    FFTToggleButton->addListener (this);

    FFTToggleButton->setBounds (0, 345, 95, 24);

    modulatedSignalToggleButton.reset (new juce::ToggleButton ("modulatedSignalToggleButton"));
    addAndMakeVisible (modulatedSignalToggleButton.get());
    modulatedSignalToggleButton->setButtonText (TRANS("modulated"));
    modulatedSignalToggleButton->addListener (this);

    modulatedSignalToggleButton->setBounds (0, 315, 95, 24);

    signalInSamePlotToggleButton.reset (new juce::ToggleButton ("signalInSamePlotToggleButton"));
    addAndMakeVisible (signalInSamePlotToggleButton.get());
    signalInSamePlotToggleButton->setTooltip (TRANS("signal in same plot"));
    signalInSamePlotToggleButton->setButtonText (juce::String());
    signalInSamePlotToggleButton->addListener (this);

    signalInSamePlotToggleButton->setBounds (120, 256, 32, 24);
    ...
    inSamePlotLabel.reset (new juce::Label ("inSamePlotLabel",
                                            TRANS("in same plot")));
    addAndMakeVisible (inSamePlotLabel.get());
    inSamePlotLabel->setFont (juce::Font (15.00f, juce::Font::plain).withTypefaceStyle ("Regular"));
    inSamePlotLabel->setJustificationType (juce::Justification::centredLeft);
    inSamePlotLabel->setEditable (false, false, false);
    inSamePlotLabel->setColour (juce::TextEditor::textColourId, juce::Colours::black);
    inSamePlotLabel->setColour (juce::TextEditor::backgroundColourId, juce::Colour (0x00000000));

    inSamePlotLabel->setBounds (120, 228, 87, 24);

    carrierInSamePlotToggleButton.reset (new juce::ToggleButton ("carrierInSamePlotToggleButton"));
    addAndMakeVisible (carrierInSamePlotToggleButton.get());
    carrierInSamePlotToggleButton->setTooltip (TRANS("carrier in same plot"));
    carrierInSamePlotToggleButton->setButtonText (juce::String());
    carrierInSamePlotToggleButton->addListener (this);

    carrierInSamePlotToggleButton->setBounds (120, 285, 32, 24);

    nodulatedInSamePlotToggleButton.reset (new juce::ToggleButton ("nodulatedInSamePlotToggleButton"));
    addAndMakeVisible (nodulatedInSamePlotToggleButton.get());
    nodulatedInSamePlotToggleButton->setTooltip (TRANS("mpdulated in same plot"));
    nodulatedInSamePlotToggleButton->setButtonText (juce::String());
    nodulatedInSamePlotToggleButton->addListener (this);

    nodulatedInSamePlotToggleButton->setBounds (120, 315, 32, 24);

    FFTInSamePlotToggleButton.reset (new juce::ToggleButton ("FFTInSamePlotToggleButton"));
    addAndMakeVisible (FFTInSamePlotToggleButton.get());
    FFTInSamePlotToggleButton->setTooltip (TRANS("FFT in same plot"));
    FFTInSamePlotToggleButton->setButtonText (juce::String());
    FFTInSamePlotToggleButton->addListener (this);

    FFTInSamePlotToggleButton->setBounds (120, 345, 32, 24);
    ...
    signalInIndividualPlotToggleButton->setBounds (216, 255, 32, 24);
    ...
    individualPlotLabel->setBounds (216, 227, 87, 24);
    ...
    carrierInIndividualPlotToggleButton->setBounds (216, 284, 32, 24);

    nodulatedIncarrierInIndiPlotToggleButton.reset (new juce::ToggleButton ("nodulatedIncarrierInIndiPlotToggleButton"));
    addAndMakeVisible (nodulatedIncarrierInIndiPlotToggleButton.get());
    nodulatedIncarrierInIndiPlotToggleButton->setTooltip (TRANS("mpdulated in individual plot"));
    nodulatedIncarrierInIndiPlotToggleButton->setButtonText (juce::String());
    nodulatedIncarrierInIndiPlotToggleButton->addListener (this);

    nodulatedIncarrierInIndiPlotToggleButton->setBounds (216, 314, 32, 24);

    FFTInnodulatedIncarrierIPlotToggleButton.reset (new juce::ToggleButton ("FFTInnodulatedIncarrierIPlotToggleButton"));
    addAndMakeVisible (FFTInnodulatedIncarrierIPlotToggleButton.get());
    FFTInnodulatedIncarrierIPlotToggleButton->setTooltip (TRANS("FFT in individual plot"));
    FFTInnodulatedIncarrierIPlotToggleButton->setButtonText (juce::String());
    FFTInnodulatedIncarrierIPlotToggleButton->addListener (this);

    FFTInnodulatedIncarrierIPlotToggleButton->setBounds (216, 344, 32, 24);
	...
	pFFTChecks->set_plot_signal(signalToggleButton->getToggleState());
	pFFTChecks->set_plot_carrier(carrierToggleButton->getToggleState());
	pFFTChecks->set_plot_modulated(modulatedSignalToggleButton->getToggleState());
	pFFTChecks->set_show_inSame_plot(inSamePlotToggleButton->getToggleState());
	pFFTChecks->set_plot_FFT(FFTToggleButton->getToggleState());
...
controlPanel::~controlPanel()
{
    ...
    signalToggleButton = nullptr;
    carrierToggleButton = nullptr;
    FFTToggleButton = nullptr;
    modulatedSignalToggleButton = nullptr;
    signalInSamePlotToggleButton = nullptr;
    toPlotLabel = nullptr;
    inSamePlotLabel = nullptr;
    carrierInSamePlotToggleButton = nullptr;
    nodulatedInSamePlotToggleButton = nullptr;
    FFTInSamePlotToggleButton = nullptr;
    signalInIndividualPlotToggleButton = nullptr;
    individualPlotLabel = nullptr;
    carrierInIndividualPlotToggleButton = nullptr;
    nodulatedIncarrierInIndiPlotToggleButton = nullptr;
    FFTInnodulatedIncarrierIPlotToggleButton = nullptr;
...
void controlPanel::buttonClicked (juce::Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == signalToggleButton.get())
    {
        //[UserButtonCode_signalToggleButton] -- add your button handler code here..
		pFFTChecks->set_plot_signal(signalToggleButton->getToggleState());
        //[/UserButtonCode_signalToggleButton]
    }
    else if (buttonThatWasClicked == carrierToggleButton.get())
    {
        //[UserButtonCode_carrierToggleButton] -- add your button handler code here..
		pFFTChecks->set_plot_carrier(carrierToggleButton->getToggleState());
        //[/UserButtonCode_carrierToggleButton]
    }
    else if (buttonThatWasClicked == FFTToggleButton.get())
    {
        //[UserButtonCode_FFTToggleButton] -- add your button handler code here..
		pFFTChecks->set_plot_FFT(FFTToggleButton->getToggleState());
        //[/UserButtonCode_FFTToggleButton]
    }
    else if (buttonThatWasClicked == modulatedSignalToggleButton.get())
    {
        //[UserButtonCode_modulatedSignalToggleButton] -- add your button handler code here..
		pFFTChecks->set_plot_modulated(modulatedSignalToggleButton->getToggleState());
        //[/UserButtonCode_modulatedSignalToggleButton]
    }
    else if (buttonThatWasClicked == signalInSamePlotToggleButton.get())
    {
        //[UserButtonCode_signalInSamePlotToggleButton] -- add your button handler code here..
        //[/UserButtonCode_signalInSamePlotToggleButton]
    }
    else if (buttonThatWasClicked == carrierInSamePlotToggleButton.get())
    {
        //[UserButtonCode_carrierInSamePlotToggleButton] -- add your button handler code here..
        //[/UserButtonCode_carrierInSamePlotToggleButton]
    }
    else if (buttonThatWasClicked == nodulatedInSamePlotToggleButton.get())
    {
        //[UserButtonCode_nodulatedInSamePlotToggleButton] -- add your button handler code here..
        //[/UserButtonCode_nodulatedInSamePlotToggleButton]
    }
    else if (buttonThatWasClicked == FFTInSamePlotToggleButton.get())
    {
        //[UserButtonCode_FFTInSamePlotToggleButton] -- add your button handler code here..
        //[/UserButtonCode_FFTInSamePlotToggleButton]
    }
    ...
    else if (buttonThatWasClicked == nodulatedIncarrierInIndiPlotToggleButton.get())
    {
        //[UserButtonCode_nodulatedIncarrierInIndiPlotToggleButton] -- add your button handler code here..
        //[/UserButtonCode_nodulatedIncarrierInIndiPlotToggleButton]
    }
    else if (buttonThatWasClicked == FFTInnodulatedIncarrierIPlotToggleButton.get())
    {
        //[UserButtonCode_FFTInnodulatedIncarrierIPlotToggleButton] -- add your button handler code here..
        //[/UserButtonCode_FFTInnodulatedIncarrierIPlotToggleButton]
    }
...
void controlPanel::displayAll()
{
	...
	signalToggleButton->setToggleState(pFFTChecks->get_plot_signal(), false);
	carrierToggleButton->setToggleState(pFFTChecks->get_plot_carrier(), false);
	modulatedSignalToggleButton->setToggleState(pFFTChecks->get_plot_modulated(), false);
	inSamePlotToggleButton->setToggleState(pFFTChecks->get_show_inSame_plot(), false);
	FFTToggleButton->setToggleState(pFFTChecks->get_plot_FFT(), false);
...
<JUCER_COMPONENT documentType="Component" className="controlPanel" componentName=""
  ...
  <TEXTEDITOR name="carrierAmplitudeTextEditor" id="dff76cd7adfac146" memberName="carrierAmplitudeTextEditor"
              ...
              readonly="0" scrollbars="1" caret="1" popupmenu="1"/>
  <TOGGLEBUTTON name="signalToggleButton" id="f73ed889b5ae397a" memberName="signalToggleButton"
                virtualName="" explicitFocusOrder="0" pos="0 257 95 24" buttonText="signal"
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
  <TOGGLEBUTTON name="carrierToggleButton" id="dd6929ff2fce5ba7" memberName="carrierToggleButton"
                virtualName="" explicitFocusOrder="0" pos="0 285 95 24" buttonText="carrier"
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
  <TOGGLEBUTTON name="FFTToggleButton" id="760272dd03666e9f" memberName="FFTToggleButton"
                virtualName="" explicitFocusOrder="0" pos="0 345 95 24" buttonText="FFT"
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
  <TOGGLEBUTTON name="modulatedSignalToggleButton" id="beecf6c5a9ba75f5" memberName="modulatedSignalToggleButton"
                virtualName="" explicitFocusOrder="0" pos="0 315 95 24" buttonText="modulated"
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
  <TOGGLEBUTTON name="signalInSamePlotToggleButton" id="91bf253e49e5ea30" memberName="signalInSamePlotToggleButton"
                virtualName="" explicitFocusOrder="0" pos="120 256 32 24" tooltip="signal in same plot"
  ...
  <LABEL name="toPlotLabel" id="12eeb64bb19698b5" memberName="toPlotLabel"
         ...
         focusDiscardsChanges="0" fontname="Default font" fontsize="15.0"
         kerning="0.0" bold="0" italic="0" justification="33"/>
  <LABEL name="inSamePlotLabel" id="35f4d0c78c8b0cd1" memberName="inSamePlotLabel"
         virtualName="" explicitFocusOrder="0" pos="120 228 87 24" edTextCol="ff000000"
         edBkgCol="0" labelText="in same plot" editableSingleClick="0"
         editableDoubleClick="0" focusDiscardsChanges="0" fontname="Default font"
         fontsize="15.0" kerning="0.0" bold="0" italic="0" justification="33"/>
  <TOGGLEBUTTON name="carrierInSamePlotToggleButton" id="f09b2a37ad1d55a6" memberName="carrierInSamePlotToggleButton"
                virtualName="" explicitFocusOrder="0" pos="120 285 32 24" tooltip="carrier in same plot"
                buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
                state="0"/>
  <TOGGLEBUTTON name="nodulatedInSamePlotToggleButton" id="527790637ecf15de"
                memberName="nodulatedInSamePlotToggleButton" virtualName="" explicitFocusOrder="0"
                pos="120 315 32 24" tooltip="mpdulated in same plot" buttonText=""
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
  <TOGGLEBUTTON name="FFTInSamePlotToggleButton" id="b87871719e0459d2" memberName="FFTInSamePlotToggleButton"
                virtualName="" explicitFocusOrder="0" pos="120 345 32 24" tooltip="FFT in same plot"
                buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
                state="0"/>
  <TOGGLEBUTTON name="signalInIndividualPlotToggleButton" id="1bfb9fc8fcf4602b"
                ...
                explicitFocusOrder="0" pos="216 255 32 24" tooltip="signal in same plot"
                buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
                state="0"/>
  <LABEL name="individualPlotLabel" id="7890dc15703afba2" memberName="individualPlotLabel"
         virtualName="" explicitFocusOrder="0" pos="216 227 87 24" edTextCol="ff000000"
  ...
  <TOGGLEBUTTON name="carrierInIndividualPlotToggleButton" id="bc39508da2594682"
                ...
                explicitFocusOrder="0" pos="216 284 32 24" tooltip="carrier in individual plot"
                buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
                state="0"/>
  <TOGGLEBUTTON name="nodulatedIncarrierInIndiPlotToggleButton" id="f567f00e93bc4c9a"
                memberName="nodulatedIncarrierInIndiPlotToggleButton" virtualName=""
                explicitFocusOrder="0" pos="216 314 32 24" tooltip="mpdulated in individual plot"
                buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
                state="0"/>
  <TOGGLEBUTTON name="FFTInnodulatedIncarrierIPlotToggleButton" id="262e405c530b61ff"
                memberName="FFTInnodulatedIncarrierIPlotToggleButton" virtualName=""
                explicitFocusOrder="0" pos="216 344 32 24" tooltip="FFT in individual plot"
                buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
                state="0"/>
</JUCER_COMPONENT>
