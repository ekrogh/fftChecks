	...
	: pFFTChecks(pFFTChks)
{
	...
	signalInCombinedPlotToggleButton->setBounds(120, 256, 32, 24);
	...
	inCombinedPlotLabel->setBounds(120, 228, 87, 24);
	...
	carrierInCombinedPlotToggleButton->setBounds(120, 285, 32, 24);
	...
	modulatedInCombinedPlotToggleButton->setBounds(120, 315, 32, 24);

	FFTInCombinedPlotToggleButton.reset(new juce::ToggleButton("FFTInCombinedPlotToggleButton"));
	addAndMakeVisible(FFTInCombinedPlotToggleButton.get());
	FFTInCombinedPlotToggleButton->setTooltip(TRANS("FFT in combined plot"));
	FFTInCombinedPlotToggleButton->setButtonText(juce::String());
	FFTInCombinedPlotToggleButton->addListener(this);

	FFTInCombinedPlotToggleButton->setBounds(120, 345, 32, 24);
	...
	modulatedInIndividualPlotToggleButton->setBounds(216, 314, 32, 24);
	...
	FFTInIndividualPlotToggleButton->setBounds(216, 344, 32, 24);
	...
	pFFTChecks->set_FFT_in_combined_plot(FFTInCombinedPlotToggleButton->getToggleState());
...
controlPanel::~controlPanel()
{
	...
	FFTInCombinedPlotToggleButton = nullptr;
...
void controlPanel::buttonClicked(juce::Button* buttonThatWasClicked)
{
	...
	else if (buttonThatWasClicked == FFTInCombinedPlotToggleButton.get())
	{
		//[UserButtonCode_FFTInCombinedPlotToggleButton] -- add your button handler code here..
		pFFTChecks->set_FFT_in_combined_plot(FFTInCombinedPlotToggleButton->getToggleState());
		//[/UserButtonCode_FFTInCombinedPlotToggleButton]
	}
...
void controlPanel::displayAll()
{
	...
	signalInCombinedPlotToggleButton->setToggleState(pFFTChecks->get_signal_in_combined_plot(), false);
	signalInIndividualPlotToggleButton->setToggleState(pFFTChecks->get_signal_in_individual_plot(), false);

	carrierToggleButton->setToggleState(pFFTChecks->get_plot_carrier(), false);
	carrierInCombinedPlotToggleButton->setToggleState(pFFTChecks->get_carrier_in_combined_plot(), false);
	carrierInIndividualPlotToggleButton->setToggleState(pFFTChecks->get_carrier_in_individual_plot(), false);

	modulatedSignalToggleButton->setToggleState(pFFTChecks->get_plot_modulated(), false);
	modulatedInCombinedPlotToggleButton->setToggleState(pFFTChecks->get_modulated_in_combined_plot(), false);
	modulatedInIndividualPlotToggleButton->setToggleState(pFFTChecks->get_modulated_in_individual_plot(), false);

	FFTToggleButton->setToggleState(pFFTChecks->get_plot_FFT(), false);
	FFTInCombinedPlotToggleButton->setToggleState(pFFTChecks->get_FFT_in_combined_plot(), false);
	FFTInIndividualPlotToggleButton->setToggleState(pFFTChecks->get_FFT_in_individual_plot(), false);
...
<JUCER_COMPONENT documentType="Component" className="controlPanel" componentName=""
  ...
  <TOGGLEBUTTON name="signalInCombinedPlotToggleButton" id="91bf253e49e5ea30"
				...
				explicitFocusOrder="0" pos="120 256 32 24" tooltip="signal in combined plot"
  ...
  <TOGGLEBUTTON name="carrierInCombinedPlotToggleButton" id="f09b2a37ad1d55a6"
				...
				explicitFocusOrder="0" pos="120 285 32 24" tooltip="carrier in combined plot"
				buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
				state="0"/>
  <TOGGLEBUTTON name="modulatedInCombinedPlotToggleButton" id="527790637ecf15de"
				...
				explicitFocusOrder="0" pos="120 315 32 24" tooltip="mpdulated in combined plot"
				buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
				state="0"/>
  <TOGGLEBUTTON name="FFTInCombinedPlotToggleButton" id="b87871719e0459d2" memberName="FFTInCombinedPlotToggleButton"
				virtualName="" explicitFocusOrder="0" pos="120 345 32 24" tooltip="FFT in combined plot"
				buttonText="" connectedEdges="0" needsCallback="1" radioGroupId="0"
				state="0"/>
  ...
  <TOGGLEBUTTON name="FFTInIndividualPlotToggleButton" id="262e405c530b61ff"
				...
				pos="216 344 32 24" tooltip="FFT in individual plot" buttonText=""
				connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
</JUCER_COMPONENT>
